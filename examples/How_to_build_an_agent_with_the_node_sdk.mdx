...
- `tools` - A list of tools the model may call. Currently, only 
  functions are supported as a tool., we'll use the `tools` array we 
  created earlier.
...
This response tells us we should call one of our functions, as it contains the following key: `finish_reason: "tool_calls"`.
...
is an object filled with the location of where the user is located right
...
2. Enjoy a cozy day at a local café or restaurant.
